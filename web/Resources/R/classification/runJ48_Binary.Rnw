...
Para mejorar la interpretabilidad del resultado, se tomó el mejor árbol encontrado por el previo análisis y se podó aquellas reglas que no superaban un criterio de confianza y soporte predefinido.  En la figura \ref{pruned} se muestra una nueva versión del árbol de clasificación preservando solo las ramas con una confianza superior a \Sexpr{str_conf} y un soporte superior a \Sexpr{str_supp}. Finalmente, la tabla \ref{rules} muestra un resumen de las reglas seleccionadas con información adicional sobre su confianza y soporte.
...
\begin{figure}[H] 
  \centering
  \href{run:\Sexpr{pdfprune}}{\includegraphics[width=1\textwidth]{\Sexpr{pdfprune}}}
  \caption{Visualización del árbol podado [Conf: \Sexpr{str_conf} Supp: \Sexpr{str_supp}].}
  \label{pruned}
\end{figure}
